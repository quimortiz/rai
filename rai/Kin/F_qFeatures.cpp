...
//  if(!limits.N)
  ...
  HALT("this is obsolete - it was penalizing limits. Better: introduce limits properly as inequalities using F_qLimits2");
...
rai::Array<rai::Joint*> getSwitchedJoints(const rai::Configuration& G0, const rai::Configuration& G1, int verbose) {
  ...
  for(rai::Frame* f: G1.frames) if((j1=f->joint) && j1->active) {
      ...
      rai::Joint* j0 = G0.frames(j1->frame->ID)->joint;
//      getJointByFrameIndices(j1->from()->ID, j1->frame->ID);
      if(!j0 || j0->from()->ID!= j1->from()->ID || j0->type!=j1->type) {
